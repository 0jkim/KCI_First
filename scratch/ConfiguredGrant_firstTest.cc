...
#include "ns3/nr-mac-scheduler-ofdma-aoi-greedy.h"
#include <random>
#include <cstdlib>
#include <ctime>
#include <iostream>
 ...
 * Global variables
 */
Time g_txPeriod = Seconds (1); // 0jkim : 패킷의 전송 주기 0.1초
...
public:
  ...
  void Setup (Ptr<NetDevice> device, Address address, uint32_t packetSize, uint32_t nPackets,
              DataRate dataRate, uint8_t period, uint32_t deadline, bool isOn);
...
MyModel::SendPacketUl () // 0jkim : UL 트래픽 패킷 전송 함수
{
  ...
  // std::cout << "\n [main] UE " << ueId << " 가 패킷을 생성한 시간 : " << creationTimeNs << " ns"
...
RxPdcpPDU (std::string path, uint16_t rnti, uint8_t lcid, uint32_t bytes, uint64_t pdcpDelay)
{
  ...
  std::cout << "Packet PDCP bytes:" << bytes << "\n";
...
void
PrintAverageThroughput (double simulationTime)
{
  std::cout << "Average Throughput per RNTI:" << std::endl;

  for (const auto &[rnti, totalBytes] : rntiToTotalBytes)
    {
      double throughput = (totalBytes * 8.0) / simulationTime; // bps
...
main (int argc, char *argv[])
{
  ...
  uint16_t numerologyBwp1 = 0; // 0jkim : Bwp1의 numerology 설정
  uint32_t packetSize = 1; // 0jkim : 패킷 사이즈 설정
  double centralFrequencyBand1 = 3550e6; // 0jkim : 중심 주파수 설정 35.5 GHz
  double bandwidthBand1 = 1e6; // 0jkim : 대역폭 설정 20 MHz
  ...
  Time sendPacketTime = Seconds (0.1); // 0jkim : 패킷 전송 시간 설정
  ...
  */
  ...
  if (sch != 0)
    {
      if (scheduler_type_number == 1)
        {
          std::cout << "Scheduler Type is RR\n";
          nrHelper->SetSchedulerTypeId (NrMacSchedulerOfdmaRR::GetTypeId ());
        }
      else if (scheduler_type_number == 2)
        {
          std::cout << "Scheduler Type is PF\n";
          nrHelper->SetSchedulerTypeId (NrMacSchedulerOfdmaPF::GetTypeId ());
        }
      else if (scheduler_type_number == 3)
        {
          std::cout << "Scheduler Type is Greedy\n";
  ...
  if (g_rxPdcpCallbackCalled &&
    ...
    {
      // PrintAverageThroughput(simTime);
