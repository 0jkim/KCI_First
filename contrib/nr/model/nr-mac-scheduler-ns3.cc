  ...
  static TypeId tid =
      TypeId ("ns3::NrMacSchedulerNs3")
          ...
          .AddAttribute ("EnableSrsInFSlots",
                         ...
                         MakeBooleanChecker ())
          .AddAttribute ("DlAmc", "The DL AMC of this scheduler", PointerValue (),
                         MakePointerAccessor (&NrMacSchedulerNs3::m_dlAmc),
                         MakePointerChecker<NrAmc> ())
          .AddAttribute ("UlAmc", "The UL AMC of this scheduler", PointerValue (),
                         MakePointerAccessor (&NrMacSchedulerNs3::m_ulAmc),
                         MakePointerChecker<NrAmc> ())
          .AddAttribute ("MaxDlMcs", "Maximum MCS index for DL", IntegerValue (-1),
          ...
          .AddAttribute ("CG", "Activate configured grant scheduling for UL periodic transmissions",
                         BooleanValue (true),
                         MakeBooleanAccessor (&NrMacSchedulerNs3::SetCG, &NrMacSchedulerNs3::GetCG),
                         MakeBooleanChecker ());
...
uint8_t
NrMacSchedulerNs3::ScheduleDlHarq (PointInFTPlane *startingPoint, uint8_t symAvail,
...
{
  NS_LOG_FUNCTION (this);
  return m_schedHarq->ScheduleDlHarq (startingPoint, symAvail, activeDlHarq, ueMap,
                                      dlHarqToRetransmit, dlHarqFeedback, slotAlloc);
}

uint8_t
NrMacSchedulerNs3::ScheduleUlHarq (PointInFTPlane *startingPoint, uint8_t symAvail,
                                   const std::unordered_map<uint16_t, UePtr> &ueMap,
...
{
  NS_LOG_FUNCTION (this);
  return m_schedHarq->ScheduleUlHarq (startingPoint, symAvail, ueMap, ulHarqToRetransmit,
                                      ulHarqFeedback, slotAlloc);
}
            ...
            if (allocation.m_symStart == symStart)
              {
                ...
                m_cqiManagement.UlSBCQIReported (expirationTime, allocation.m_tbs, params,
                                                 UeInfoOf (*itUe), allocation.m_rbgMask,
...
template <typename T>
std::vector<T>
NrMacSchedulerNs3::MergeHARQ (std::vector<T> *existingFeedbacks, const std::vector<T> &inFeedbacks,
...
NrMacSchedulerNs3::ProcessHARQFeedbacks (
    std::vector<T> *harqInfo, const NrMacSchedulerUeInfo::GetHarqVectorFn &GetHarqVectorFn,
                                   ...
                                   std::deque<VarTtiAllocInfo> *allocations) const
{
  ...
  for (uint8_t sym = symStart; sym < symStart + numSymToAllocate; ++sym)
    {
      allocations->emplace_front (VarTtiAllocInfo (std::make_shared<DciInfoElementTdma> (
          sym, 1, mode, DciInfoElementTdma::CTRL, rbgBitmask)));
                                  ...
                                  std::deque<VarTtiAllocInfo> *allocations) const
{
  ...
  for (uint8_t sym = symStart; sym < symStart + numSymToAllocate; ++sym)
    {
      allocations->emplace_back (VarTtiAllocInfo (std::make_shared<DciInfoElementTdma> (
          sym, 1, mode, DciInfoElementTdma::CTRL, rbgBitmask)));
      NS_LOG_INFO ("Allocating CTRL symbol, type"
                   << mode << " in TDMA. numSym=1, symStart=" << static_cast<uint32_t> (sym)
                   << " Remaining CTRL sym to allocate: " << sym - symStart);
          ...
          for (const auto &it : dci->m_tbSize)
            {
              ...
              for (uint16_t numLc = 0; numLc < distributedBytes.size (); numLc++)
                {
                  bytesPerLcPerStream.at (numLc).emplace_back (Assignation (
                      distributedBytes.at (numLc).m_lcg, distributedBytes.at (numLc).m_lcId,
      ...
      for (const auto &ue : beam.second)
        {
          ...
          if (m_cgScheduling)
            ...
            {
              dci = CreateUlDci (spoint, ue.first,
                                 symPerBeam.at (GetBeam (
                                     beam))); // nr-mac-scheduler-ofdma.cc의 CreateUlDci를 사용한다.
            }
              ...
              if (GetBandwidthInRbg () - spoint->m_rbg <= 1)
                {
                  ...
                  spoint->m_sym =
                      spoint->m_sym +
                      ue.first->m_ulSym; //FlexOFDMA has to start after n_ulSym of on UE before
                }
            }
 ...
 */
void
NrMacSchedulerNs3::DoScheduleUlSr (PointInFTPlane *spoint,
                                   const std::list<uint16_t> &rntiList) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (spoint->m_rbg == 0);
  ...
  for (const auto &v : rntiList)
    {
      ...
      // On/Off [5]
      // std::cout << "[mac-scheduler-ns3] : Assign 12 bytes to UE " << v << " because of a SR\n";
    }
}
                               ...
                               const std::vector<DlHarqInfo> &dlHarqFeedback)
{
  ...
  DoScheduleDl (dlHarqFeedback, activeDlHarq, &activeDlUe, params.m_snfSf, ulAllocations,
                &dlSlot.m_slotAllocInfo, params.m_slotType);
 ...
 */
uint8_t
NrMacSchedulerNs3::DoScheduleUl (const std::vector<UlHarqInfo> &ulHarqFeedback, const SfnSf &ulSfn,
                                 SlotAllocInfo *allocInfo, LteNrTddSlotType type)
{
  ...
  if (activeUlHarq.size () > 0)
    {
      uint8_t usedHarq = ScheduleUlHarq (&ulAssignationStartPoint, ulSymAvail, m_ueMap,
                                         &m_ulHarqToRetransmit, ulHarqFeedback, allocInfo);
          ...
          if (alloc.m_dci->m_type == DciInfoElementTdma::DATA)
            {
              ...
              allocations.emplace_back (AllocElem (
                  alloc.m_dci->m_rnti, alloc.m_dci->m_tbSize.at (0), alloc.m_dci->m_symStart,
                  alloc.m_dci->m_numSym, alloc.m_dci->m_mcs.at (0), alloc.m_dci->m_rbgBitmask));
            }
        }
    }
  ...
  for (uint32_t i = 0; i < m_srsCtrlSymbols; ++i)
    {
      ...
      auto dci = std::make_shared<DciInfoElementTdma> (
          rnti, DciInfoElementTdma::UL, spoint->m_sym, 1, mcs, tbs, ndi, rv,
          DciInfoElementTdma::SRS, GetBwpId (), GetTpc ());
...
NrMacSchedulerNs3::DoScheduleDl (const std::vector<DlHarqInfo> &dlHarqFeedback,
                                 const ActiveHarqMap &activeDlHarq, ActiveUeMap *activeDlUe,
                                 const SfnSf &dlSfnSf, const SlotElem &ulAllocations,
                                 SlotAllocInfo *allocInfo, LteNrTddSlotType type)
{
  NS_LOG_INFO (this);
  ...
  if (activeDlHarq.size () > 0)
    {
      uint8_t usedHarq = ScheduleDlHarq (&dlAssignationStartPoint, dlSymAvail, activeDlHarq,
                                         m_ueMap, &m_dlHarqToRetransmit, dlHarqFeedback, allocInfo);
...
{
  NS_LOG_FUNCTION (this);
  // std::cout << "DoschedUlSrInfoReq\n";
  // Merge RNTI in our current list
  for (const auto &ue : params.m_srList)
    {
      if (params.m_aoiMap.find (ue) != params.m_aoiMap.end ())
        {
          uint32_t aoiValue = params.m_aoiMap.at (ue);
          m_tempAoiMap[ue] = aoiValue; // AoI 임시 저장
          m_tempWMA[ue] = params.WMA_sap.at(ue);
          // std::cout<<"dd "<<params.WMA_sap.at(ue)<<std::endl;
          // std::cout << "DoschedUlSrInfoReq에서 Ue " << ue << "에 대한 AoI[" << aoiValue
          //           << "]를 임시로 저장\n";
        }
...
void
NrMacSchedulerNs3::DoScheduleUlresources_configuredGrant (PointInFTPlane *spoint,
                                                          const std::list<uint16_t> &rntiList) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (spoint->m_rbg == 0);
              ...
              while (bufSizeUeIt != m_bufCgr.end ())
                {
                  ...
                  NS_LOG_DEBUG ("Assigning " << bufSize << " bytes to UE " << v
                                             << " because of a CGR");
...
void
NrMacSchedulerNs3::DoSchedUlCgrInfoReq (
    const NrMacSchedSapProvider::SchedUlCgrInfoReqParameters &params)
{
  NS_LOG_FUNCTION (this);
