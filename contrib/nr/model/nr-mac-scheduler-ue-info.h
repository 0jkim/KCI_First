...
public:
  ...
  std::vector<uint8_t>
      m_dlMcs; //!< DL MCS per stream, it is initialized with a starting MCS upon UE addition to gNB and the scheduler
  uint8_t m_ulMcs{0}; //!< UL MCS

  std::vector<uint32_t> m_dlTbSize{
      0}; //!< DL Transport Block Size per stream, depends on MCS and RBG, updated in UpdateDlMetric()
  uint32_t m_ulTbSize{
...
private:
  const GetRbPerRbgFn
      m_getNumRbPerRbg; //!< Function that points to a method which knows the number of RB per RBG.
};
