...
public:
  ...
  void SetLastPacketCreationTime (uint64_t time); // 가장 최근 패킷 생성 시간을 설정하는 메서드
  uint64_t GetLastPacketCreationTime (void) const; // 가장 최근 패킷 생성 시간을 반환하는 메서드

private:
  uint16_t m_rnti{0}; //!< RNTI
  uint64_t m_lastpacketCreationTime{0}; // 가장 최근 패킷 생성 시간 저장 멤버 변수
};
...
private:
  uint32_t
      m_k; //!< delay (in slots) between DL/UL DCI reception and subframe to which it applies for reception/transmission of Data (k0/k2)
  uint32_t
      m_k1; //!< delay (in slots) between DL Data reception and subframe to which it applies for Harq feedback (k1)
...
private:
  uint32_t
      m_k; //!< delay (in slots) between UCI reception and subframe to which it applies for reception/transmission of Data (k2)
