...
public:
  ...
  bool GetCG () const;
  void
  SetAge (uint16_t ueRnti, uint32_t age)
  {
    m_tempAoiMap[ueRnti] = age;
  } // age 설정 함수
  uint32_t
  GetAge (uint16_t ueRnti) const
  {
    auto it = m_tempAoiMap.find (ueRnti);
    if (it != m_tempAoiMap.end ())
      {
        return it->second;
      }
    else
      {
        // 해당 RNTI의 AoI 값이 존재하지 않는 경우 기본값 (예: 0) 반환
        std::cout << "RNTI " << ueRnti << "에 대한 AoI 값이 존재하지 않습니다.\n";
        return 0;
      }
  } // age 반환 함수
  uint32_t
  GetWMA(uint16_t ueRnti) const
  {
    auto it = m_tempWMA.find(ueRnti);
    if(it != m_tempWMA.end())
    {
      return it->second;
    }
...
protected:
  std::unordered_map<uint16_t, uint32_t> m_tempAoiMap; //!< RNTI에 대한 AoI 임시 저장소
  std::unordered_map<uint16_t, uint32_t>m_tempWMA;
  /**
  ...
  virtual uint8_t ScheduleDlHarq (NrMacSchedulerNs3::PointInFTPlane *startingPoint,
                                  uint8_t symAvail, const ActiveHarqMap &activeDlHarq,
...
private:
  ...
  struct SlotElem
  {
     ...
     */
    SlotElem (uint8_t totUlSym) : m_totUlSym (totUlSym)
    {
    }
  ...
  template <typename T>
  std::vector<T> MergeHARQ (std::vector<T> *existingFeedbacks, const std::vector<T> &inFeedbacks,
  ...
  uint8_t DoScheduleDlData (PointInFTPlane *spoint, uint32_t symAvail, const ActiveUeMap &activeDl,
                            SlotAllocInfo *slotAlloc) const;
  uint8_t DoScheduleUlData (PointInFTPlane *spoint, uint32_t symAvail, const ActiveUeMap &activeUl,
                            SlotAllocInfo *slotAlloc) const;
  ...
  //Configured Grant
  void DoScheduleUlresources_configuredGrant (PointInFTPlane *spoint,
