...
NrUePhy::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::NrUePhy")
          ...
          .AddAttribute ("TxPower", "Transmission power in dBm", DoubleValue (2.0),
                         MakeDoubleAccessor (&NrUePhy::m_txPower), MakeDoubleChecker<double> ())
          .AddAttribute (
              "NoiseFigure",
              ...
              DoubleValue (
                  5.0), // nr code from NYU and UniPd assumed in the code the value of 5dB, thats why we configure the default value to that
          ...
          .AddAttribute (
              ...
              MakeEnumChecker (
                  NrSpectrumValueHelper::UNIFORM_POWER_ALLOCATION_BW, "UniformPowerAllocBw",
          ...
          .AddAttribute ("LBTThresholdForCtrl",
                         ...
                         MakeTimeAccessor (&NrUePhy::m_lbtThresholdForCtrl), MakeTimeChecker ())
          .AddAttribute ("TbDecodeLatency", "Transport block decode latency",
                         TimeValue (MicroSeconds (100)),
                         MakeTimeAccessor (&NrPhy::SetTbDecodeLatency, &NrPhy::GetTbDecodeLatency),
                         MakeTimeChecker ())
          .AddAttribute ("EnableUplinkPowerControl",
                         "If true, Uplink Power Control will be enabled.", BooleanValue (false),
                         MakeBooleanAccessor (&NrUePhy::SetEnableUplinkPowerControl),
                         MakeBooleanChecker ())
          .AddAttribute ("FixedRankIndicator", "The rank indicator", UintegerValue (1),
          ...
          .AddAttribute ("UseFixedRi",
                         ...
                         "streams",
                         BooleanValue (true), MakeBooleanAccessor (&NrUePhy::UseFixedRankIndicator),
                         MakeBooleanChecker ())
          .AddAttribute (
              "RiSinrThreshold1",
          ...
          .AddAttribute (
              ...
              MakeDoubleChecker<double> ())
          .AddTraceSource ("DlDataSinr", "DL DATA SINR statistics.",
                           MakeTraceSourceAccessor (&NrUePhy::m_dlDataSinrTrace),
                           "ns3::NrUePhy::DlDataSinrTracedCallback")
          .AddTraceSource ("DlCtrlSinr", "Report the SINR computed for DL CTRL",
          ...
          .AddAttribute ("NrSpectrumPhyList", "List of all SpectrumPhy instances of this NrUePhy.",
                         ObjectVectorValue (), MakeObjectVectorAccessor (&NrUePhy::m_spectrumPhys),
          ...
          .AddAttribute ("CG", "Activate configured grant scheduling for UL periodic transmissions",
                         BooleanValue (true),
                         MakeBooleanAccessor (&NrUePhy::SetCG, &NrUePhy::GetCG),
                         MakeBooleanChecker ())
          .AddAttribute (
              "ConfigurationTime", "Time required to configure UE with configured grant signal",
              UintegerValue (10),
              MakeUintegerAccessor (&NrUePhy::SetConfigurationTime, &NrUePhy::GetConfigurationTime),
              MakeUintegerChecker<uint8_t> ())
          .AddAttribute ("CGPeriod", "The periodicity of configured grant transmissions",
                         UintegerValue (10),
                         MakeUintegerAccessor (&NrUePhy::SetCGPeriod, &NrUePhy::GetCGPeriod),
                         MakeUintegerChecker<uint8_t> ());
  return tid;
}
                                        ...
                                        uint8_t activeStreams)
{
  ...
  m_reportPowerSpectralDensity (m_currentSlot, txPsd, numSym * GetSymbolPeriod (), m_rnti, m_imsi,
                                GetBwpId (), GetCellId ());
  ...
  if (msg->GetMessageType () == NrControlMessage::DL_DCI)
    {
      ...
      m_phyUeRxedDlDciTrace (m_currentSlot, GetCellId (), m_rnti, GetBwpId (),
                             dciInfoElem->m_harqProcess, dciMsg->GetK1Delay ());
          ...
          if (m_cgScheduling)
            {
              ...
              uint8_t number_slots_configuration =
                  (configurationTime * numberOfSlot_insideOneSubframe) -
  ...
  else if (msg->GetMessageType () == NrControlMessage::RAR)
    {
      ...
      Simulator::Schedule ((GetSlotPeriod () * (GetL1L2CtrlLatency () / 2)), &NrUePhy::DoReceiveRar,
                           this, rarMsg);
    }
  else
    {
  ...
  if (m_tddPattern.size () == 0)
    {
      ...
      VarTtiAllocInfo dlCtrlSlot (std::make_shared<DciInfoElementTdma> (
          0, m_dlCtrlSyms, DciInfoElementTdma::DL, DciInfoElementTdma::CTRL, rbgBitmask));
  ...
  if (m_tddPattern[currentSlotN] > LteNrTddSlotType::DL)
    {
      ...
      VarTtiAllocInfo ulCtrlSlot (std::make_shared<DciInfoElementTdma> (
          GetSymbolsPerSlot () - m_ulCtrlSyms, m_ulCtrlSyms, DciInfoElementTdma::UL,
          ...
          if (m_dciGranted[cg_slot_counter_futureTx] == nullptr)
          ...
          else
            {
              ...
              auto it = m_dci_cg_map.find (m_currentSlot.GetEncForStreamWithSymStart (
                  streamId, m_dciGranted[cg_slot_counter_futureTx]->m_symStart));
              ...
              if (it == m_dci_cg_map.end ())
              ...
              else
                {
                  ...
                  m_sfnsfConfiguredGrantPeriod.Add (numberOfSlot_insideOneSubframe *
                                                    configuredGrant_periodicity);
                  ...
                  NS_LOG_INFO ("Sending a packet to PHY layer in slot "
                               << m_sfnsfConfiguredGrantPeriod);

                  uint64_t key = m_sfnsfConfiguredGrantPeriod.GetEncForStreamWithSymStart (
                      streamId, m_dciGranted[cg_slot_counter_futureTx]->m_symStart);
                  ...
                  if (it_1 == m_dci_cg_map.end ())
                    {
                      it_1 =
                          m_dci_cg_map
                              .insert (std::make_pair (key, m_dciGranted[cg_slot_counter_futureTx]))
                              .first;
...
NrUePhy::UlSrs (const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  for (uint8_t streamIndex = 0; streamIndex < m_spectrumPhys.size (); streamIndex++)
    {
      m_phyTxedCtrlMsgsTrace (m_currentSlot, GetCellId (), dci->m_rnti, GetBwpId (),
                              *srsMsg.begin ());
      m_spectrumPhys.at (streamIndex)
          ->StartTxUlControlFrames (srsMsg, varTtiPeriod - NanoSeconds (1.0));
    }
      ...
      if (dci->m_tbSize.at (streamIndex) > 0)
        {
          ...
          //are iterating over
          m_spectrumPhys.at (streamIndex)
              ->AddExpectedTb (dci->m_rnti, dci->m_ndi.at (streamIndex),
                               dci->m_tbSize.at (streamIndex), dci->m_mcs.at (streamIndex),
                               FromRBGBitmaskToRBAssignment (dci->m_rbgBitmask), dci->m_harqProcess,
                               dci->m_rv.at (streamIndex), true, dci->m_symStart, dci->m_numSym,
                               m_currentSlot);
...
NrUePhy::UlData (const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  NS_LOG_DEBUG ("UE" << m_rnti << " TXing UL DATA frame for" << " symbols " << +dci->m_symStart
                     << "-" << +(dci->m_symStart + dci->m_numSym - 1) << "\t start "
                     << Simulator::Now () << " end " << (Simulator::Now () + varTtiPeriod));

  Simulator::Schedule (NanoSeconds (1.0), &NrUePhy::SendDataChannels, this, pktBurst, ctrlMsg,
...
NrUePhy::EndVarTti (const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  if (m_currSlotAllocInfo.m_varTtiAllocInfo.size () == 0)
  ...
  else
    {
      ...
      if (m_cgScheduling)
        {
          if (dci->m_type == DciInfoElementTdma::DATA && dci->m_format == DciInfoElementTdma::UL &&
              m_ulPacketToTransmit == false)
            {
              m_dciGranted[cg_slot_counter] = std::make_shared<DciInfoElementTdma> (
                  dci->m_rnti, dci->m_format, dci->m_symStart, dci->m_numSym, dci->m_mcs,
                  dci->m_tbSize, dci->m_ndi, dci->m_rv, dci->m_type, dci->m_bwpIndex,
                  dci->m_harqProcess, dci->m_rbgBitmask, dci->m_tpc);

              uint8_t streamId = 0;
              SfnSf dataSfn_cg = m_currentSlot;
              uint8_t number_slots_for_processing_configurationPeriod =
                  7; //5 in the mac 7 in the phy becasue it has 2 slots of processing time in phy
              ...
              uint64_t key = dataSfn_cg.GetEncForStreamWithSymStart (
                  streamId, m_dciGranted[cg_slot_counter]->m_symStart);

              auto it = m_dci_cg_map.find (key);

              if (it == m_dci_cg_map.end ())
                {
                  it = m_dci_cg_map.insert (std::make_pair (key, m_dciGranted[cg_slot_counter]))
                           .first;
...
{
  Simulator::ScheduleWithContext (m_netDevice->GetNode ()->GetId (), GetTbDecodeLatency (),
                                  &NrUePhySapUser::ReceivePhyPdu, m_phySapUser, p);
  // m_phySapUser->ReceivePhyPdu (p);
}

void
NrUePhy::SendDataChannels (const Ptr<PacketBurst> &pb,
                           const std::list<Ptr<NrControlMessage>> &ctrlMsg, const Time &duration)
{
  if (pb->GetNPackets () > 0)
    {
                               ...
                               uint8_t harqProcessId, uint8_t rv)
{
  ...
  if (m_dlHarqInfo.m_harqStatus.empty ()) // No Harq reported yet, initialize the structure
    {
      ...
      m_dlHarqInfo.m_harqStatus = std::vector<enum DlHarqInfo::HarqStatus> (
          m_spectrumPhys.size (), DlHarqInfo::HarqStatus::NONE);
  ...
  NS_ASSERT (streamId < m_dlHarqInfo.m_harqStatus.size () &&
             m_dlHarqInfo.m_harqStatus.at (streamId) == DlHarqInfo::HarqStatus::NONE);
...
{
  NS_LOG_FUNCTION (this);
  Simulator::ScheduleWithContext (nodeId, MilliSeconds (0), &NrUePhy::StartEventLoop, this, frame,
                                  subframe, slot);
}
