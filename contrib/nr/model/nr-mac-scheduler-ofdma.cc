  ...
  if (countLessThan7B == ueInfo->m_dlTbSize.size ())
    {
      NS_LOG_DEBUG ("While creating DCI for UE " << ueInfo->m_rnti << " assigned "
                                                 << ueInfo->m_dlRBG << " DL RBG, but TBS < 7");
      return nullptr;
    }
...
NrMacSchedulerOfdma::AssignULRBG (uint32_t symAvail, const ActiveUeMap &activeUl) const
{
  ...
  if (m_schType_OFDMA == 1)
    {
      for (const auto &el : activeUl) // activeUl은 각 빔에 속한 ue들을 나타내는 것 같음.
        {
          // Distribute the RBG evenly among UEs of the same beam
          uint32_t beamSym = symPerBeam.at (GetBeamId (el));
          uint32_t rbgAssignable = 1 * beamSym;
          // std::cout<<"가용 리소스 블록 확인 : "<<rbgAssignable<<std::endl;
          // 각 빔당 가용 리소스 블록은 12임
          std::vector<UePtrAndBufferReq> ueVector;
          FTResources assigned (0, 0);
          const std::vector<uint8_t> ulNotchedRBGsMask = GetUlNotchedRbgMask ();
          uint32_t resources =
              ulNotchedRBGsMask.size () > 0
          ...
          // std::sort (ueVector.begin (), ueVector.end (),
          //            [] (const NrMacSchedulerNs3::UePtrAndBufferReq &a,
          //                const NrMacSchedulerNs3::UePtrAndBufferReq &b) {
          //              // UE 포인터가 NrMacSchedulerUeInfoAoiGreedy 타입인지 확인하고, 해당 타입이라면 CalculateMetric을 사용하여 비교
          //              auto ueInfoA =
          //                  std::dynamic_pointer_cast<NrMacSchedulerUeInfoAoiGreedy> (a.first);
          //              auto ueInfoB =
          //                  std::dynamic_pointer_cast<NrMacSchedulerUeInfoAoiGreedy> (b.first);

          //              if (ueInfoA && ueInfoB)
          //                {
          //                  return ueInfoA->CalculateMetric () > ueInfoB->CalculateMetric ();
          //                }

          //              // 만약 타입이 다르면 기본적인 비교를 유지 (혹은 다른 방식으로 처리할 수도 있음)
          //              return false; // 기본 비교
          //            });
          while (resources > 0)
            {
              ...
              std::sort (
                  ueVector.begin (), ueVector.end (),
                  GetUeCompareUlFn ()); //Comment out this line to assign the packets in order
              // 정렬 후 UE Vector 출력
              // std::cout << "After sorting UE Vector:" << std::endl;
              // for (const auto &ue : ueVector)
              //   {
              //     auto uePtr = std::dynamic_pointer_cast<NrMacSchedulerUeInfoAoiGreedy> (ue.first);
              //     if (uePtr)
              //       {
              //         std::cout << "RNTI: " << uePtr->GetRnti () << ", AoI: " << uePtr->GetAoi ()
              //                   << ", WMA:"<<uePtr->GetNiceCount()<< std::endl;
              //       }
              //       else{
              //         std::cout<<"RNTI: "<<ue.first->GetRnti()<<std::endl;
              //       }
              //   }
              auto schedInfoIt = ueVector.begin ();
      ...
      for (const auto &el : activeUl)
        {
                          ...
                          if (!alreadyAssigned)
                            {
                              ...
                              posRBassignable++;
                              v_rbgAssignable[posRBassignable] =
                                  rbgInOneSymbolPrime / rbgAssignable;
                              posRBassignable++;

                              std::cout << "Assigned RBs: First = "
                                        << v_rbgAssignable[posRBassignable - 1]
                                        << " and Second = " << v_rbgAssignable[posRBassignable - 2]
                                        << '\n';
                            }
              ...
              for (uint8_t ii = 0; ii <= v_rbgAssignable.size (); ii++)
                {
                  if (v_rbgAssignable[ii] == 0)
                    ...
                    {
                      int numRbgAssignable =
                          (rbgInOneSymbolPrime / (ueVector.size ())) / v_rbgAssignable[ii];
                      ...
                      rbAssignableMin =
                          uint32_t (v_rbgAssignable[ii] * numRbgAssignable * numSym - rbPacket);
                          ...
                          if (numSym < 12)
                            {
                              ...
                              std::cout << "Assignable RBs: " << rbgAssignable
                                        << " we are going to loss " << rbAssignableMin
                                        << " resources" << '\n';
                            }
                        }
                    }
          ...
          while (resources > 0)
                                      ...
                                      if ((GetUe (*schedInfoIt)->m_ulSym + initSym - 1) < sym)
                                        {
                                          ...
                                          while (UeScheduling != ueSchedVector.end ())
                                            {
                                              if (GetUe (*schedInfoIt)->m_rnti ==
                                                  (GetUeRnti (*UeScheduling)))
                                                {
                                                  goto scheduleUE;
                                                }
                                              UeScheduling++;
                                          ...
                                          if (scheduledUEs == vectorSizeOfUe)
                                            {
                                              if (scheduledUEs > 1)
                                                {
                                                  if (resources == (rbgInOneSymbol * beamSym) -
                                                                       (rbgInOneSymbol * (sym - 1)))
                                                    {
                                  ...
                                  if (applyConstrain)
                                    {
                                      ...
                                      while (schedInfoUeIt != ueVector.end ())
                                        {
                                          ...
                                          while (UeScheduling != ueSchedVector.end ())
                                            {
                                              if (GetUe (*schedInfoUeIt)->m_rnti ==
                                                  (GetUeRnti (*UeScheduling)))
                                                {
                                                  if (resources - 1 >=
                                                      ((rbgInOneSymbol * beamSym) -
                                                       (rbgInOneSymbol * (sym - 1))))
                                                    {
                                                      sym = sym - 1;
                                                    }
                                                  if (GetUe (*schedInfoUeIt)->m_ulRBG <
                                                      ((sym - initSym + 1) *
                                                       GetRBcounter (*UeScheduling))) //+1 ¿?
                                                    {
                                                      GetUe (*schedInfoUeIt)->m_ulRBG =
                                                          (sym - initSym + 1) *
                                                          GetRBcounter (*UeScheduling);
                                                      GetUe (*schedInfoUeIt)->m_ulSym =
                                                          (sym - initSym + 1);
                                                      assigned.m_rbg =
                                                          GetUe (*schedInfoUeIt)->m_ulRBG;
                                                      assigned.m_sym =
                                      ...
                                      resources =
                                          (rbgInOneSymbol * beamSym) - (rbgInOneSymbol * sym);
                                      sym++;
                                      clearSchedVector = true;
                                    }
                      ...
                      if (GetUeRnti (*UeScheduling) == GetUe (*schedInfoIt)->m_rnti)
                        {
                          if (GetFirstSym (*UeScheduling_sym) == sym)
                            ...
                            {
                              GetUe (*schedInfoIt)->m_ulSym =
                                  (sym - (GetFirstSym (*UeScheduling_sym))) + 1;
                      ...
                      if (GetFirstSym (*UeScheduling_sym) == sym)
                        {
                          ...
                          GetUe (*schedInfoIt)->m_ulRBG += rbgAssignable;
                          assigned.m_rbg += rbgAssignable;
                          ueSchedVector.emplace_back (GetUe (*schedInfoIt)->m_rnti,
                                                      GetUe (*schedInfoIt)->m_ulRBG);
                          GetUeRBGcounter (*UeScheduling_sym) = ueSchedVector;
                          if (resources > rbgAssignable)
                            {
                              resources -= rbgAssignable;
            ...
            UEscheduled:
              ...
              NS_LOG_DEBUG ("Assigned " << GetUe (*schedInfoIt)->m_ulRBG << " UL RBG, spanned over "
                                        << uint32_t (GetUe (*schedInfoIt)->m_ulSym)
                                        << " SYM, to UE " << GetUe (*schedInfoIt)->m_rnti);
              ...
              if (resources == 0)
                {
                  ...
                  if (m_schType_OFDMA == 2 &&
                      (ueSchedVectorFirstSym.size () > 1 || ueSchedVector.size () > 1))
                    {
                      GetUe (*schedInfoIt)->m_ulTbSize = 0;
                      GetUe (*schedInfoIt)->m_ulSym = 0;
                      GetUe (*schedInfoIt)->m_ulRBG = 0;
...
{
  NS_LOG_FUNCTION (this);
  // std::cout << "test ofdma createuldci \n";
  ...
  std::shared_ptr<DciInfoElementTdma> dci = std::make_shared<DciInfoElementTdma> (
      ueInfo->m_rnti, DciInfoElementTdma::UL, spoint->m_sym, (ueInfo->m_ulSym), ulMcs, ulTbs, ndi,
      rv, DciInfoElementTdma::DATA, GetBwpId (), GetTpc ());
