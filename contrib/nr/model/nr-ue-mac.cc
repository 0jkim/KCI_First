...
std::unordered_map<uint16_t, uint64_t> uemac_tempUeTimeMap;
///////////////////////////////////////////////////////////
// SAP forwarders
///////////////////////////////////////////////////////////
...
//-----------------------------------------------------------------------
void // 최신 패킷 생성으로 업데이트하는 메서드 구현
NrUeMac::UpdateLastPacketCreationTime (uint64_t creationTime)
{
  NS_LOG_FUNCTION (this << creationTime);
  m_lastPacketCreationTime = creationTime;
}
...
NrUeMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  // if (params.pdu->PeekPacketTag (creationTimeTag))
  //   {
  //     uint64_t packetCreationTime = creationTimeTag.GetCreationTime ();
  //     UpdateLastPacketCreationTime (packetCreationTime);
  //     std::cout << "[ue-mac] 최신 패킷의 생성시간으로 업데이트 함 패킷 생성 시간은 "
  //               << packetCreationTime << "\n";
  //     NS_LOG_INFO ("Transmitting packet with creation time: " << packetCreationTime);
  //   }

  // uint64_t transmissionTime = Simulator::Now ().GetNanoSeconds ();
  // PacketTransmissionTimeTag transmissionTimeTag;
  // transmissionTimeTag.SetTransmissionTime (transmissionTime);
  // params.pdu->AddPacketTag (transmissionTimeTag);
  // //////////////////////////////////////////////////////////////////////////////
  ...
  // CQI 정보를 기반으로 한 동적 지연 계산
  // Time dynamicDelay = CalculateDynamicDelay ();
  // 동적 지연을 적용하여 패킷 전송을 스케줄링
  // Simulator::Schedule (dynamicDelay, &NrPhySapProvider::SendMacPdu, m_phySapProvider, params.pdu,
  //                       m_ulDciSfnsf, m_ulDci->m_symStart, params.layer);
...
// 테스트딜레이
Time
NrUeMac::CalculateDynamicDelay () const
{
  // CQI를 기반으로 동적 지연 계산
  // 이 부분은 실제 구현에 따라 달라질 수 있습니다.
  uint8_t cqi = GetLatestCqi (); // 최신 CQI 값을 가져오는 함수 (구현 필요)
...
uint8_t
NrUeMac::GetLatestCqi () const
{
  return m_latestCqi;
}
void
NrUeMac::UpdateLatestCqi (uint8_t cqi)
{
  NS_LOG_FUNCTION (this << static_cast<uint32_t> (cqi));
  m_latestCqi = cqi;
}
//////////////////////////////////////////////
...
NrUeMac::SendSR () const
{
  ...
  msg->SetSourceBwp (GetBwpId ());
  msg->SetRNTI (m_rnti);
  // msg->SetLastPacketCreationTime (m_lastPacketCreationTime); // SR 메시지에 패킷 생성 시간을 포함
  msg->SetLastPacketCreationTime (uemac_tempUeTimeMap[m_rnti]); // 테스트
  // On/Off [2]
  // std::cout << "[ue-mac] UE " << m_rnti << "가 SR을 보냄 : " << uemac_tempUeTimeMap[m_rnti]
  //           << "ns\n";
...
NrUeMac::DoReceivePhyPdu (Ptr<Packet> p)
{
  ...
  //std::cout << "[ue-mac] DoReceivePhyPdu인데 Phy에서 패킷을 받는거 같은데 정확히 몰라서 출력\n";
  PacketCreationTimeTag creationTimeTag;
  ...
  switch (msg->GetMessageType ())
    {
      case (NrControlMessage::UL_DCI): {
        // On/Off [7]
        // std::cout << "[ue-mac] ue "<<m_rnti<<"가 dci 메시지를 받음. 시각 : "<<Simulator::Now().GetNanoSeconds()<<std::endl;;
