...
public:
  // cqi 딜레이 테스트
  uint8_t GetLatestCqi () const;
  Time CalculateDynamicDelay () const;
  void UpdateLatestCqi (uint8_t cqi); // cqi 업데이트
   ...
   */
  typedef void (*RxedUeMacCtrlMsgsTracedCallback) (const SfnSf sfnSf, const uint16_t nodeId,
   ...
   * \param [in] ctrlMessage the pointer to msg to get the msg type
   */
  typedef void (*TxedUeMacCtrlMsgsTracedCallback) (const SfnSf sfnSf, const uint16_t nodeId,
                                                   const uint16_t rnti, const uint8_t bwpId,
                                                   Ptr<NrControlMessage> ctrlMessage);
...
private:
  void
  UpdateLastPacketCreationTime (uint64_t creationTime); // 최근 패킷 생성 시간 업데이트 하는 함수
  uint64_t m_lastPacketCreationTime{0}; // 가장 최근 패킷 생성 시간을 저장할 멤버변수
  uint8_t m_latestCqi{0}; // 최신 CQI 값을 저장할 변수
  /**
   ...
   */
  void
  DoReportBufferStatus_configuredGrant (LteMacSapProvider::ReportBufferStatusParameters params);
  /**
...
private:
  ...
  SrCgMachine m_srState_configuredGrant{
      INACTIVE_CG}; //!< Default state for the configured grant state machine.

  std::shared_ptr<DciInfoElementTdma>
      m_dciGranted_stored[100]; //!< The stored DCI-s for CG allocation.
