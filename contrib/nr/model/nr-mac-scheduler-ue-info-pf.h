...
public:
   ...
   */
  NrMacSchedulerUeInfoPF (float alpha, uint16_t rnti, BeamConfId beamConfId,
                          const GetRbPerRbgFn &fn)
      : NrMacSchedulerUeInfo (rnti, beamConfId, fn), m_alpha (alpha)
  {
  }
  ...
  double m_potentialTputDl{
      0.0}; //!< Potential throughput in downlink in one assignable resource (can be a symbol or a RBG)
  ...
  double m_potentialTputUl{
      0.0}; //!< Potential throughput in uplink in one assignable resource (can be a symbol or a RBG)
};
