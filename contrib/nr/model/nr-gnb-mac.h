...
#include "aoi-tag.h" // 0jkim : AoI 태그 클래스 헤더 파일 포함
...
class NrRarMessage;
class BeamConfId;
class NrMacSchedulerUeInfoAoiGreedy;
////////////// 전역변수 시작//////////////////
struct UeAoiInfo
{
  uint64_t lastPacketCreationTime; // SR 보낸 시점의 패킷 생성 시간
  uint64_t beforeSchedulingTime; // 스케줄링 되기전의 시간
  uint64_t currentAoI;
  bool lastTransmissionSuccessful;
  uint32_t WMA = 1;
  std::shared_ptr<NrMacSchedulerUeInfoAoiGreedy> ueInfoPtr; // UE 정보에 대한 포인터 추가
};
////////////// 전역변수 끝//////////////////
/**
 * \ingroup gnb-mac
...
public:
  void CalculateAndPrintFinalAoI (); // 시스템 평균 aoi 계산 메서드
  void PrintFinalAoI ();
...
private:
  std::unordered_map<uint16_t, UeAoiInfo> m_ueAoiMap; // UE별 aoi정보들을 저장
  std::vector<uint64_t> m_scheduledAgeValues; // 모든 AoI 값을 저장
  std::vector<uint64_t> m_totalAge;
  std::vector<uint64_t> m_totaluenum;
